"""Batch send v2.5.0 update email to all users."""
...
async def main():
    ...
    async for db in get_db():
        ...
        emails = [row[0] for row in result.all() if row[0] and row[0] not in SKIP]
        break
    await close_db()

    total = len(emails)
    print(f"Sending to {total} users...")
    success = 0
    failed = 0
    ...
    print(f"\nDone: {success} sent, {failed} failed, {total} total")
